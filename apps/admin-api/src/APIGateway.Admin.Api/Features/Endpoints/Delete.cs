...
/// <summary>
/// DELETE <c>/admin/v1/endpoints/{id}</c> — soft-delete (active=0).
/// 204 sucesso, 404 nao existe. Requer role <c>operator</c>.
/// </summary>
/// <remarks>
/// Paridade com <c>handlers/endpoints.go:DeleteEndpoint</c>. Targets
/// permanecem; gateway Go rejeita roteamento quando
/// <c>proxy_endpoints.active = 0</c>.
/// </remarks>
public static class Delete
{
    public static RouteGroupBuilder MapEndpointsDelete(this RouteGroupBuilder group)
    {
        group.MapDelete("/{id:long}", HandleAsync)
            .WithName("EndpointsDelete")
            .WithTags("Endpoints")
        ...
        try
        {
            ...
            await audit.WriteAsync(
                new AuditEvent(
                    ...
                    EventType: "endpoint_deleted",
                    Severity: "info",
                    Metadata: new { endpoint_id = id, was_already_inactive = affected == 0 }),
                ct);

            logger.LogInformation("endpoint_deleted id={Id} idempotent={Idem}", id, affected == 0);
            return Results.NoContent();
        }
        catch (Exception ex)
        {
            logger.LogError(ex, "endpoint_delete_failed id={Id}", id);
            return ApiErrorResults.Internal("failed to delete endpoint");
        }
    }
